...
// AddProduct.tsx - UPDATED VERSION WITH DESIGN DETAILS
import React, { useState, useEffect } from 'react';
import {
  Box,
  Button,
...
const steps = ['Service Details', 'Design Details & Status', 'Review'];
...
export default function AddProduct() {
  ...
  const [activeStep, setActiveStep] = useState(0);
  const [filteredSubcategories, setFilteredSubcategories] = useState(subcategories);
  const [features, setFeatures] = useState<Array<{name: string, description: string}>>([{name: '', description: ''}]);
  ...
  const handleSubmit = () => {
    // Combine formData with features
    const completeData = {
      ...formData,
      features: features.filter(f => f.name.trim() !== '' || f.description.trim() !== '')
    };
    
    console.log('Project data:', completeData);
    alert('Interior project added successfully!');
              ...
              <Box sx={{ 
                ...
                <Box sx={{ 
                  ...
                  <TextField
                    ...
                    onChange={handleInputChange}
                    size={isMobile ? "small" : "medium"}
                    placeholder="e.g., White & Wood, Grey"
                    helperText="Color scheme"
                    InputProps={{
                      startAdornment: <ColorLensIcon sx={{ mr: 1, color: 'text.secondary', fontSize: 20 }} />
                    }}
                  />
                </Box>
                ...
                <Box sx={{ 
                  ...
                  <TextField
                    ...
                    name="shutterFinish"
                    value={formData.shutterFinish}
                    onChange={handleInputChange}
                    size={isMobile ? "small" : "medium"}
                    placeholder="e.g., Glossy, Matte, Wood Finish"
                    helperText="Finish type for shutters"
                  />
                  <TextField
                    fullWidth
                    ...
                    onChange={handleInputChange}
                    size={isMobile ? "small" : "medium"}
                    placeholder="e.g., Granite, Quartz, Marble"
                    helperText="Countertop material type"
                    InputProps={{
                      startAdornment: <CountertopsIcon sx={{ mr: 1, color: 'text.secondary', fontSize: 20 }} />
                    }}
                  />
                  <TextField
                    fullWidth
                    ...
                    onChange={handleInputChange}
                    size={isMobile ? "small" : "medium"}
                    placeholder="e.g., Small Families, Bachelors"
                  ...
                  >
                    <Box sx={{ flex: 1, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                      <TextField
                        ...
                        placeholder="e.g., Pull-out Tray, Soft-close Hinges"
                      />
                      <TextField
                        fullWidth
                        ...
                        multiline
                        rows={isMobile ? 2 : 1}
                        sx={{ flex: 2, minWidth: { xs: '100%', sm: 300 } }}
        ...
        return (
          ...
          >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              {/* Category & Basic Specifications Section */}
              <Box>
                <Typography variant="subtitle2" color="primary" gutterBottom sx={{ 
                  display: 'flex', 
                  alignItems: 'center', 
                  ...
                  <Box sx={{ 
                    ...
                    <Box>
                      ...
                      <Typography variant="body1" sx={{ 
                        ...
                        {formData.shutterFinish || 'Not specified'}
                      </Typography>
                    </Box>
                    <Box>
                      <Typography variant="caption" color="text.secondary" display="block" gutterBottom>
                      ...
                      <Typography variant="body1" sx={{ 
                        ...
                        {formData.countertopMaterial || 'Not specified'}
                      </Typography>
                    </Box>
                    <Box>
                      <Typography variant="caption" color="text.secondary" display="block" gutterBottom>
                      ...
                      <Typography variant="body1" sx={{ 
                        ...
                        {formData.idealFor || 'Not specified'}
                      </Typography>
                    </Box>
                  </Box>
                </Box>
              ...
              <Box>
                ...
                {features.filter(f => f.name.trim() !== '' || f.description.trim() !== '').length > 0 ? (
                  <Stack spacing={2}>
                    {features.map((feature, index) => (
                        ...
                        >
                          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                            ...
                            <Box sx={{ flex: 2, minWidth: { xs: '100%', sm: 300 } }}>
                              ...
                              <Typography variant="body1">
                                {feature.description || 'Not specified'}
                              </Typography>
                            </Box>
                          </Box>
                        </Box>
