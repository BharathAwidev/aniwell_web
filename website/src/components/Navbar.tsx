...
// Map dynamic category names to static routes
...
// Function to get static route or use dynamic slug
const getRoute = (mainSlug: string, subSlug: string): string => {
  // Check if this is a "More" category item that should go to static page
  const moreCategorySlugs = ['more', 'information', 'company'];
  
  if (moreCategorySlugs.includes(mainSlug.toLowerCase())) {
    // Check for static page mapping
    const staticRoute = STATIC_PAGE_MAP[subSlug.toLowerCase()];
    if (staticRoute) {
      return staticRoute;
    }
  }
  
  // Default: Use dynamic route
...
const Navbar = () => {
      ...
      },
      {
        ...
        subCategories: [
          ...
          },
          {
            ...
            name: "Policies",
            slug: "policies",
            description: "Our terms and privacy policies",
            status: "YES"
          }
        ]
      }
  ...
  const handleSubmenuClick = (mainSlug: string, subSlug: string) => {
    const route = getRoute(mainSlug, subSlug);
    navigate(route);
    setIsMenuOpen(false);
    setMobileOpenMenu(null);
  };
