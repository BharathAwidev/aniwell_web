...
const slides = [
  {
    type: "video",
    source: video, // Add .mp4 extension
    title: "Home to beautiful interiors",
    subtitle: "India's Most Trusted Brand",
    buttonText: "BOOK FREE CONSULTATION",
    videoLength: 8, // 8 seconds video length
  },
  {
    type: "image",
    image: "https://images.unsplash.com/photo-1586023492125-27b2c045efd7?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80",
...
const HomeCarousel = () => {
  const [current, setCurrent] = useState(0);
  const [showForm, setShowForm] = useState(false);
  const autoSlideTimer = useRef<ReturnType<typeof setTimeout> | null>(null); // Fixed type
  ...
  const resetAutoSlideTimer = () => {
    ...
    const intervalDuration = currentSlide.type === "video" 
      ? currentSlide.videoLength * 1000 // Use video length for video slides
      : 5000; // Default 5 seconds for image slides
