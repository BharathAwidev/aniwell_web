...
// Map for "More" category subitems to static pages
const STATIC_PAGE_MAP: Record<string, { 
  route: string; 
  ...
  'policies': {
    ...
    description: 'View our terms, privacy policy, and guidelines',
    color: 'bg-purple-500'
  },
  'policy': {
    route: '/policies',
...
const CategoryPage = () => {
  ...
  // Check if this is a "More" category (static pages category)
  const isMoreCategory = (slug: string): boolean => {
    const moreCategorySlugs = ['more', 'information', 'company', 'info'];
    return moreCategorySlugs.includes(slug.toLowerCase());
  };
    ...
    const fetchCategoryData = async () => {
      ...
      try {
        ...
        const sectionSubcategories = allSubcategories
          ...
          .map((sub: SubcategoryData, index: number) => {
            // For "More" category, don't add design counts
            if (isMoreCategory(category)) {
              return {
                id: sub.id || `sub-${index}`,
                name: sub.name,
                slug: sub.name?.toLowerCase().replace(/\s+/g, '-') || `sub-${index}`,
                description: sub.description || `Learn more about ${sub.name}`,
                status: sub.status || "YES",
                designs: 0, // No design counts for static pages
                image: getCategoryImage(sub.name)
              };
            }
        ...
        const transformedCategory: Category = {
          ...
          name: currentCategory.name,
          slug: currentCategory.name?.toLowerCase().replace(/\s+/g, '-') || "",
          description: isMoreCategory(category) 
            ? "Learn more about our company, get in touch with our team, and review our policies and terms."
            : "We bring you carefully-curated interior design ideas, to give your home a brand new look. Explore exclusive interior designs and trends that are every bit inspirational as they are practical. Our team of interior designers have put together ideas across kitchen, bedroom, living room and more, to help you pick a design that will best suit your home interior requirements.",
          hasDropdown: sectionSubcategories.length > 0,
          status: currentCategory.status || "YES",
          subCategories: sectionSubcategories
  ...
  const getCategoryImage = (categoryName: string): string => {
    ...
    const imageMap: Record<string, string> = {
      ...
      'pooja room': 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=800&auto=format&fit=crop',
      'dining room': 'https://images.unsplash.com/photo-1615529328331-f8917597711f?w=800&auto=format&fit=crop',
      // For "More" category items
      'about': 'https://images.unsplash.com/photo-1552664730-d307ca884978?w=800&auto=format&fit=crop',
      'contact': 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800&auto=format&fit=crop',
      'policies': 'https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=800&auto=format&fit=crop'
    };
  ...
  // Handle click for "More" category items (redirect to static pages)
  const handleStaticPageClick = (subSlug: string) => {
    const pageInfo = STATIC_PAGE_MAP[subSlug.toLowerCase()];
    if (pageInfo) {
      navigate(pageInfo.route);
      return;
    }
    
    // If not found in map, try partial match
    for (const [key, info] of Object.entries(STATIC_PAGE_MAP)) {
      if (subSlug.toLowerCase().includes(key) || key.includes(subSlug.toLowerCase())) {
        navigate(info.route);
        return;
      }
    }
    
    // Fallback: navigate to subcategory page
    navigate(`/${category}/${subSlug}`);
  };

  // Get icon and color for "More" category items
  const getStaticPageInfo = (subSlug: string) => {
    const pageInfo = STATIC_PAGE_MAP[subSlug.toLowerCase()];
    if (pageInfo) {
      return pageInfo;
    }
    ...
    // Default info
    return {
      route: `/${category}/${subSlug}`,
      icon: <Info className="w-6 h-6" />,
      description: `Learn more about ${subSlug}`,
      color: 'bg-gray-500'
    };
  };
  ...
  const getFallbackCategory = (categorySlug: string): Category | null => {
    const fallbackCategories: Record<string, Category> = {
      ...
      'more': {
        ...
        name: "More",
        slug: "more",
        description: "Learn more about our company, get in touch with our team, and review our policies and terms. We're here to provide you with the information and support you need.",
        hasDropdown: true,
        status: "YES",
        subCategories: [
          {
            ...
            status: "YES",
            designs: 0,
            image: getCategoryImage('about')
          },
          {
            id: "5",
            name: "Contact Us",
            slug: "contact-us",
            description: "Get in touch with our team for inquiries, support, or to start your project.",
            status: "YES",
            designs: 0,
            image: getCategoryImage('contact')
          },
          {
            id: "6",
            name: "Policies",
            slug: "policies",
            description: "Review our terms of service, privacy policy, and other important guidelines.",
            status: "YES",
            designs: 0,
            image: getCategoryImage('policies')
          }
        ]
      },
      'projects': {
  ...
  const isMore = isMoreCategory(categoryData.slug);
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-gray-50 rounded-2xl p-8 mb-12">
          <div className="flex items-center gap-3 mb-6">
            <div className={`w-8 h-8 ${isMore ? 'bg-blue-50' : 'bg-orange-50'} rounded-lg flex items-center justify-center`}>
              <Sparkles className={`w-4 h-4 ${isMore ? 'text-blue-500' : 'text-orange-500'}`} />
            </div>
            <h1 className="text-3xl md:text-4xl font-semibold text-gray-900">
              {categoryData.name}
            </h1>
        ...
        {categoryData.subCategories && categoryData.subCategories.length > 0 ? (
          <div className="mb-8">
            <h2 className="text-2xl font-semibold text-gray-900 mb-8">
              {isMore ? 'Company Information' : 'Explore Designs'}
            </h2>
            <div className={`grid ${isMore ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'} gap-8`}>
              {categoryData.subCategories.map((subCategory) => {
                const staticPageInfo = getStaticPageInfo(subCategory.slug);
                
                return isMore ? (
                  // "More" category items (static pages)
                  <button
                    key={subCategory.id}
                    onClick={() => handleStaticPageClick(subCategory.slug)}
                    className="group bg-white rounded-xl overflow-hidden border border-gray-100 hover:border-blue-200 transition-all duration-300 hover:shadow-lg text-left"
                  >
                    <div className="relative h-48 overflow-hidden">
                      ...
                      <div className="absolute top-4 left-4">
                        <div className={`${staticPageInfo.color} text-white p-2 rounded-lg`}>
                          {staticPageInfo.icon}
                        </div>
                      </div>
                    </div>
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <ArrowRight className="w-5 h-5 text-gray-300 group-hover:text-blue-600 transition-colors duration-300" />
                      </div>
                  ...
                  // Regular category items (design subcategories)
                  <Link
                    key={subCategory.id}
                    to={`/${categoryData.slug}/${subCategory.slug}`}
                    className="group bg-white rounded-xl overflow-hidden border border-gray-100 hover:border-orange-200 transition-all duration-300 hover:shadow-lg"
          ...
          <div className="bg-gray-50 rounded-2xl p-12 text-center">
            <div className={`w-16 h-16 ${isMore ? 'bg-blue-50' : 'bg-orange-50'} rounded-full flex items-center justify-center mx-auto mb-6`}>
              <Sparkles className={`w-8 h-8 ${isMore ? 'text-blue-500' : 'text-orange-500'}`} />
            </div>
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Content Coming Soon</h2>
            <p className="text-gray-600 max-w-md mx-auto">
              {isMore 
                ? "We're working on adding more information to this section."
                : `We're currently working on amazing designs for ${categoryData.name}.`}
            </p>
          </div>
        )}

        {/* Bottom Section - Only show for non-"More" categories */}
        {!isMore && categoryData.subCategories && categoryData.subCategories.length > 0 && (
          <div className="mt-16 text-center">
            <h3 className="text-2xl font-semibold text-gray-900 mb-6">
              Need help with your interior design?
            </h3>
            ...
            <Link 
              to="/get-quote" 
              className="bg-orange-500 hover:bg-orange-600 text-white font-medium py-3 px-8 rounded-lg transition-all duration-300 hover:shadow-lg inline-block"
            >
              Book Free Consultation
            </Link>
          </div>
